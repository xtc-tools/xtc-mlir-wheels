...
CCACHE_OPTS=""
[ "$BUILD_LLVM_CCACHE" != 1 ] || CCACHE_OPTS="-DLLVM_CCACHE_BUILD=ON"

if [ "$BUILD_LLVM_TOOLS" = 1 ]; then
    MLIR_TOOLS="-DLLVM_INCLUDE_TOOLS=ON -DLLVM_BUILD_TOOLS=ON -DLLVM_INCLUDE_UTILS=ON -DLLVM_BUILD_UTILS=ON -DLLVM_INSTALL_UTILS=ON -DLLVM_INSTALL_TOOLCHAIN_ONLY=OFF"
else
    MLIR_TOOLS="-DLLVM_INCLUDE_TOOLS=ON -DLLVM_BUILD_TOOLS=OFF -DLLVM_INCLUDE_UTILS=OFF -DLLVM_BUILD_UTILS=OFF -DLLVM_INSTALL_UTILS=OFF -DLLVM_INSTALL_TOOLCHAIN_ONLY=ON"
fi

if [ "$BUILD_LLVM_MLIR_BINDINGS" = 1 ]; then
    MLIR_BINDINGS="-DLLVM_INCLUDE_TOOLS=ON -DMLIR_ENABLE_BINDINGS_PYTHON=ON -DMLIR_LINK_MLIR_DYLIB=OFF"
else
    MLIR_BINDINGS="-DLLVM_INCLUDE_TOOLS=ON -DMLIR_ENABLE_BINDINGS_PYTHON=OFF -DMLIR_LINK_MLIR_DYLIB=ON"
fi
