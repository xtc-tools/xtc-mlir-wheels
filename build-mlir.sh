...
if [ "$BUILD_LLVM_MLIR_BINDINGS" = 1 ]; then
    MLIR_BINDINGS="-DMLIR_ENABLE_BINDINGS_PYTHON=ON -DMLIR_LINK_MLIR_DYLIB=OFF -DMLIR_PYTHON_STUBGEN_ENABLED=ON"
else
    MLIR_BINDINGS="-DMLIR_ENABLE_BINDINGS_PYTHON=OFF -DMLIR_LINK_MLIR_DYLIB=ON -DMLIR_PYTHON_STUBGEN_ENABLED=OFF"
fi
